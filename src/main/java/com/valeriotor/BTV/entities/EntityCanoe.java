...
//This class was pretty much a big copy & paste from the Vanilla EntityBoat, with minor changes.
public class EntityCanoe extends EntityBoat{
	
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityCanoe.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityCanoe.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityCanoe.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityCanoe.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] {EntityDataManager.createKey(EntityCanoe.class, DataSerializers.BOOLEAN), EntityDataManager.createKey(EntityCanoe.class, DataSerializers.BOOLEAN)};
    ...
    protected void entityInit()
    {
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityCanoe.Type.OAK.ordinal()));
        ...
        else if (!this.world.isRemote && !this.isDead)
        {
            ...
            else
            {
                ...
                if (flag || this.getDamageTaken() > 40.0F)
                {
                    ...
                    {
                        this.dropItemWithOffset(this.getItemCanoe(), 1, 0.0F);
                    }
    ...
    {
        if (entityIn instanceof EntityCanoe)
        {
    ...
    public Item getItemCanoe()
    {
        return ItemRegistry.canoe;
    }
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityCanoe.Status.UNDER_WATER && this.status != EntityCanoe.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityCanoe.Status getCanoeStatus()
    {
        EntityCanoe.Status entityboat$status = this.getUnderwaterStatus();
        ...
        {
            return EntityCanoe.Status.IN_WATER;
        }
        else
        {
            float f = this.getCanoeGlide();

            if (f > 0.0F)
            {
                this.boatGlide = f;
                return EntityCanoe.Status.ON_LAND;
            }
            else
            {
                return EntityCanoe.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityCanoe.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)BlockLiquid.getLiquidHeight(iblockstate, this.world, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityCanoe.Status entityboat$status = EntityCanoe.Status.UNDER_FLOWING_WATER;
                                return entityboat$status;
                            }
        ...
        return flag ? EntityCanoe.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityCanoe.Status.IN_AIR && this.status != EntityCanoe.Status.IN_AIR && this.status != EntityCanoe.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityCanoe.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityCanoe.Status.IN_WATER)
            {
                d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityCanoe.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityCanoe.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityCanoe.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityCanoe.Status.ON_LAND)
            {
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityCanoe.Status.ON_LAND)
                    {
    ...
    public void setCanoeType(EntityCanoe.Type boatType)
    {
     ...
     * Pigs, Horses, and Canoes are generally "steered" by the controlling passenger.
     */
    @Nullable
