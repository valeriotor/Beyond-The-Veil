...
public class BTVLanguageProvider extends LanguageProvider {
    ...
    private void addBloodThesis() {
        ...
        add("research.thesis.risks.text.2", "Rituals facing high instability could quickly turn into nightmares for the practitioner: various harmful events have been reported, including explosions, lightning strikes, and even the appearance of undead creatures, captivated by the ritual for obvious reasons.\\n" +
                "The greatest risk, of course, was an abrupt end to the ritual, signifying a loss of the ingredients and the waste of a life.");
        ...
        add("research.thesis.modifier.text.1", "Despite the associated risks, some practitioners deviated from the rigid recipe prescribed by the ritual descriptions, revealing a small degree of flexibility within their setup. Some rituals could thus be slightly modified in order to change their effects or the process of the sacrifice itself.\\n" +
                "To do this, additional offerings were placed on the blood basins surrounding the altar. During the linking process, the modifier ingredients would have to be linked (in any order) just after the initial knife cut and before any of the other offerings. Below, we list some of the more common such modifiers.");
        add("research.thesis.modifier.section.2", "4.2 Modifiers");
        add("research.thesis.modifier.text.2", "• Reducing instability: a sculk catalyst was used to reduce the risk of adverse effects occurring, starting from the risk of offerings being knocked off in the first place.\\n" +
                "• Returning ingredients: a gold ingot, a block of quartz, or an emerald could each be used as an offering, and were said to provide a chance of some other offering in the ritual not being fully burned by the blood nucleus, allowing it to be salvaged. All three ingredients could be used together to increase this chance, though each could only be used once.\\n" +
                "• Targeting locations: some rituals allowed for their effects to take place at specific locations. To do this, either a pathway sigil or immortal sigil could be used, the latter specifying the target individual’s current location (if within the same dimension). We believe the reported use of immortal sigils points in all likelihood to the existence of immortal humans other than Saint Adelina Delduomo.");
