...
public class PlayerTickEvents {
    ...
    public static void tickEvent(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END && event.side == LogicalSide.SERVER) {
            event.player.getCapability(PlayerDataProvider.PLAYER_DATA, null).ifPresent(playerData -> {
                ...
                for (CounterType type : counterTypes) {
                    if (type instanceof WaypointType waypointType) {
                        Messages.sendToPlayer(GenericToClientPacket.removeWaypoint(waypointType), (ServerPlayer) event.player);
                    }
                }
            });
        }
